...
comp_aerea = st.multiselect(label = 'Filtre por companhia aérea', options = df_view_cia['razao_social'], default = None)

geo_fig = go.Figure(layout={'width':800, 'height' : 800})

df_view_cia_filt = df_view_cia[df_view_cia['razao_social'].isin(comp_aerea)].reset_index(drop=True)
...
with placeholder.container():
    ...
    for i in range(len(df_view_cia_filt)):
        geo_fig.add_trace(
            go.Scattergeo(
                locationmode = 'ISO-3',
                lon = [df_view_cia_filt['longitude_origem'][i], df_view_cia_filt['longitude_destino'][i]],
                lat = [df_view_cia_filt['latitude_origem'][i], df_view_cia_filt['latitude_destino'][i]],
                mode = 'lines',
