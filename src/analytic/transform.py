...
def view_cia(df_air_cia: pd.DataFrame, df_vra: pd.DataFrame, df_aerodromos:pd.DataFrame) -> pd.DataFrame:
    ...
    df_aerodromos_1 = df_aerodromos.loc[:,['icao', 'name', 'state', 'country_iso']]
    ...
    df_ranking_cia = (df_air_cia.merge(df_vra, 'left', left_on= 'icao', right_on= 'icao_empresa_aerea', suffixes=('', '_vra'))
                                ...
                                .groupby(['razao_social','icao_origem', 'aeroporto_origem', 'estado_uf_origem', 'icao_destino', 'aeroporto_destino', 'estado_uf_destino'], as_index=False)['razao_social']
                                .agg(['count'])
                               # .sort_values(by=['razao_social', 'count'], axis = 0, ascending=[True, False])                                                        
                                )
    
    
    df_ranking_cia['rank'] = df_ranking_cia.groupby('razao_social')['count'].rank(method='dense', ascending=False)
